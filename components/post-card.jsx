    ...
    const shareHandler = async () => {
        ...
        if (navigator.share) {
            navigator
                ...
                .catch((err) => {
                    alert(
                        `Error: ${err} (If you're on Firefox, please enable webshare)`
                    );
                });
        } else {
            alert(
                `Error: ${err} (Looks like your browser doesn't support webshare or you are not on an HTTPS domain! If you are on a Firefox-based browser, please enable webshare in the settings or config.)`
            );
        }
        // try {
    ...
    return (
        <div className="card-post-result inline-flex flex-col md:flex-row w-80 max-w-80 min-w-max h-auto md:w-4/5 md:max-w-none md:min-w-0 md:h-32 lg:h-48 overflow-hidden my-4 rounded-md bg-gray-900 border-2 border-slate-700 hover:border-slate-300">
            ...
            <div className="content flex flex-col justify-between gap-2 md:justify-start w-[300px] md:w-full px-4 py-2 md:py-4 lg:py-8 lg:px-8">
                <a href={href}>
                    <h3
                        className={`hover:underline /*decoration-transparent*/`}
                    >
                        {title}
                    </h3>
                </a>
                <p className="w-full h-24 text-sm lg:text-base md:h-auto overflow-hidden text-ellipsis mb-2">
                    {desc}
                </p>
            </div>
            ...
            <div className="buttons flex flex-row md:flex-col items-center whitespace-nowrap border-t md:border-l border-slate-700 w-[300px] md:w-24">
                <span
                    className="flex justify-center items-center w-full h-full hover-accent-bg p-2 lg:p-4 cursor-pointer"
